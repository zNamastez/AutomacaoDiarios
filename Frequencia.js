...
(async () => {
    ...
    const turnos = await page.evaluate(() => {
        ...
        const selectElement = document.querySelector('#cphFuncionalidade_cphCampos_ddlTurno');
        const optionValues = [];
    ...
    for (let i = 0; i < turnos.length; i++) {
        ...
        await page.select('#cphFuncionalidade_cphCampos_ddlTurno', turnos[i].value);
        ...
        for (let n = 2; n <= turmas; n++) {

            await Delay(5000)
            await page.select('#cphFuncionalidade_cphCampos_ddlTurno', turnos[i].value);
            await Listar()

            const serie = `#cphFuncionalidade_gdvListagem > tbody > tr:nth-child(${n})`;
            await PageWaitSelector(serie);
            await page.click(serie);
            await Delay(5000);
            
            const frequencia = '#cphFuncionalidade_btnAuxiliar2';
            await PageWaitSelector(frequencia)
            await page.click(frequencia);
